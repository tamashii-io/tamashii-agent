...
RSpec.describe Tamashi::Agent::Master do
  ...
  describe "#initialize" do
    ...
    it 'creates all components' do
      expect_any_instance_of(described_class).to receive(:create_component).with(Tamashi::Agent::Connection, any_args)
      expect_any_instance_of(described_class).to receive(:create_component).with(Tamashi::Agent::Buzzer, any_args)
      expect_any_instance_of(described_class).to receive(:create_component).with(Tamashi::Agent::CardReader, any_args)
      subject
    end
  ...
  describe "#process event" do
    let(:master_only_events) { [Tamashi::Agent::EVENT_SYSTEM_COMMAND] }
    ...
    context "when the connection is not ready" do
      before do
        subject.process_event(Tamashi::Agent::EVENT_CONNECTION_NOT_READY, "ABC")
      end
      it_behaves_like "broadcast to components", Tamashi::Agent::EVENT_BEEP, "error"
    end
