...
RSpec.describe Codeme::Agent::Adapter::Base do
  ...
  describe ".current_class" do
    ...
    context "when env is test" do
      before do
        allow(Codeme::Agent::Config).to receive(:env).and_return("test")
      end
    ...
    context "when env is not test" do
      before do
        allow(Codeme::Agent::Config).to receive(:env).and_return("production")
      end
